...
object QuakeMLParserSpec extends Spec:
  ...
  def spec =
    suite("A QuakeMLParser")(
      test("It should process a minimal element set (event, origin, magnitude)") {
        val quakemlStream = ZStream.fromPath(Paths.get("src/test/resources/quakeml/moho-usp-2023-01.xml"))
        for
      ...
      test("It should process a big quakeml (10K events).") {
        ...
        for
          stream <- QuakeMLParser().parse(header ++ hugeStream).orDie
